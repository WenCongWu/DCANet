...
parser.add_argument('--src_dir', default='./Datasets/RENOIR', type=str, help='Directory for full resolution images')
parser.add_argument('--tar_dir', default='./Datasets/train', type=str, help='Directory for image patches')
parser.add_argument('--ps', default=140, type=int, help='Image Patch Size')
...
#get sorted folders
files = natsorted(glob(os.path.join(src, '*', '*', '*.bmp')))
...
def save_files(i):
    ...
    for j in range(NUM_PATCHES):
        ...
        cv2.imwrite(os.path.join(noisy_patchDir, '{}_{}.bmp'.format(i+1, j+1)), noisy_patch)
        cv2.imwrite(os.path.join(clean_patchDir, '{}_{}.bmp'.format(i+1, j+1)), clean_patch)
